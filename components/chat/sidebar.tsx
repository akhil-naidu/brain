  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1 px-2 pt-2 pb-2">
        ...
        <button
          className="flex h-9 w-full cursor-pointer items-center gap-2 rounded-md px-2 text-sm text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
          onClick={onNewChat}
          type="button"
        >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2">
        ...
        {hasActiveSession ? (
          <div
            aria-current="page"
            className="rounded-md bg-muted/50 px-2 py-2 text-sm text-foreground"
          >
            <span className="line-clamp-2">{sessionTitle}</span>
          </div>
        ) : (
          <p className="px-2 py-2 text-sm text-muted-foreground/70">No messages yet</p>
        )}
      </div>
    </aside>
  );
