...
const TOKEN_PREFIX = "brain-http-mcp-proxy.v1.";
...
export function mintHttpMcpProxyToken(
  input: { readonly connectionId: string; readonly workspaceId: string | null },
...
export function verifyHttpMcpProxyToken(
  token: string,
  env: { readonly [key: string]: string | undefined } = process.env,
  now = Date.now(),
): { readonly connectionId: string; readonly workspaceId: string | null } | null {
  const secret = signingSecret(env);
  ...
  try {
    ...
    const connectionRaw = "c" in parsed ? parsed.c : undefined;
    ...
    return { connectionId: connectionRaw.trim(), workspaceId };
  } catch {
    return null;
  }
}
