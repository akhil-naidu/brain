...
`scripts/start-production.mjs` starts eve from `.output/server/index.mjs`, waits for the port, then boots Next. If eve never binds `:4274`, chat fails with `ECONNREFUSED 127.0.0.1:4274`.
...
4. Persist **`.eve/`** if you use Menu Connect (MCP tokens are file-based)
...
- [Upgrades and backups](/docs/self-hosting/upgrades-and-backups) — releases and Postgres  
