...
Product-level guardrails (workspaces, HITL approvals, Ask mode, agent safety posture) are described in [Architecture](/docs/self-hosting/architecture) and [Instance policies](/docs/enterprise/instance-policies).
...
8. **Agent safety posture** — Leave **Auto** unless you intend Strict (more pauses) or Dangerous (fewer pauses). Command policy still blocks high-risk shell and SQL.  
