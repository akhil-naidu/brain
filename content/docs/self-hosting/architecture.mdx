...
- Mutating MCP tools wait for in-chat approval in the default **Auto** posture; instance admins can choose Strict or Dangerous on [Instance policies](/docs/enterprise/instance-policies). **Ask** mode cannot call tools. A command policy still blocks high-risk shell and SQL. Auto and Strict also screen tool results before the model sees them.
