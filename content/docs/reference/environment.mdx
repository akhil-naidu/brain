...
Brain pins **microsandbox** for Agent coding. There is no Brain-specific sandbox API key in `.env.example`. Hosts may need the microsandbox platform install or **`MSB_PATH`** pointing at the binary — see [Attached repo](/docs/chat/attached-repo).
...
| Slack inbound (Events / HITL) | `SLACK_BOT_TOKEN`, `SLACK_SIGNING_SECRET` |
