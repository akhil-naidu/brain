...
Brain is not a full IDE (no LSP or inline apply). In **Agent** mode you can attach a GitHub repo so the agent clones it into a sandbox and works on files there.
...
Brain is not a full IDE — no LSP, no inline apply. Review the PR on GitHub like any other change.
