...
Credentials and grants are **per signed-in user × active workspace**. Enabling a tool for one chat does not share another user’s tokens. Brain calls the live API at request time — it does **not** copy Slack, ClickUp, or warehouse tables into a local search index. See [Architecture](/docs/self-hosting/architecture).
...
Static OAuth / PAT / HTTP apps can store secrets at **workspace**, **host**, or **env** scope. See [BYOA credentials](/docs/connections/byoa-credentials).
