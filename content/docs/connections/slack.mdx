...
Optional: **Allowed channels** in the same inbound dialog (or env `SLACK_INBOUND_CHANNEL_IDS`). Leave the list empty to allow every channel the bot can see. One `C…` / `G…` id or `#name` per line. DMs always work. @mentions and thread follow-ups outside the list are ignored (no reply), including leftover active threads. Saving an empty box stores “unrestricted” and ignores env.
...
- Optional channel allowlist limits @mentions; DMs are not filtered
