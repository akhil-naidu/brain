---
title: dFlow
description: Connect dFlow via official MCP with dynamic client registration.
---
...
```text
{BRAIN_PUBLIC_URL}/api/connections/dflow/callback
```

Use your real origin (not `localhost`) in production. Mid-turn auth may print an exact redirect URI under `.eve/`.

Sign in picks one organisation. Read tools list apps, environments, services, logs, and templates.
