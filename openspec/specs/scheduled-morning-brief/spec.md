...
The chat UI SHALL expose morning-brief schedule controls from a composer Schedules control available during any chat (including empty chat). Schedules SHALL open as an inline panel above the composer (not a modal dialog or floating popover). A Manage control SHALL open a dedicated `/schedules` page for roomier management. The chat sidebar MUST show a short schedules preview (including morning brief status) and a Manage/View more control that opens `/schedules`. Those controls SHALL present the morning brief as its own section (separate from playbook schedules), let the user enable it with an on/off switch, set local time and weekdays when enabled, optionally enable Slack delivery with a channel target when Slack is on, and run a brief once. Detailed time/Slack fields SHOULD stay collapsed while the schedule is disabled. Copy SHOULD stay short. Saving MUST happen as the user edits, with brief Saved feedback. The schedule UI MUST NOT show host crontab, curl, or other operator setup commands. The empty chat state MUST NOT duplicate the full morning-brief schedule panel.
...
#### Scenario: Off by default keeps details collapsed

- **WHEN** the morning brief schedule is disabled
- **THEN** the UI shows an on/off control and does not require time or Slack setup to leave it off

#### Scenario: Run now from Schedules

- **WHEN** the user chooses to run the morning brief once from Schedules
