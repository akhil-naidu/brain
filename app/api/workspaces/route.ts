...
export async function GET() {
  ...
  const active = await workspaces.resolveActiveWorkspace(session.userId);
  const list = await workspaces.listWorkspacesForUser(session.userId);
  const activeMembership = list.find((item) => item.id === active.id);
  const isInstanceAdmin = await workspaces.isInstanceAdmin(session.userId);
...
export async function POST(request: Request) {
  ...
  if (!(await workspaces.isInstanceAdmin(session.userId)) && !policies.allowCreateWorkspace) {
    return NextResponse.json(
